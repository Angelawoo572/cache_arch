...
Keep executable scripts directly under `scripts/` so existing commands such as `bash scripts/run_nn_replay.sh` keep working. This README groups them by role.
...
- `profile_bypass_pcs.py` — analyze an access-trace CSV and generate `configs/bypass/bypass_pc_list*.txt`.
...
- `run_bypass.sh` — LRU vs LRU+bypass using a PC list from `configs/bypass/`.
