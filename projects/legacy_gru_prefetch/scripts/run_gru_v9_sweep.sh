...
#
# Usage:
#   bash projects/legacy_gru_prefetch/scripts/run_gru_v9_sweep.sh
#
# Optional:
#   TRACES="605.mcf_s-994B 619.lbm_s-4268B" bash projects/legacy_gru_prefetch/scripts/run_gru_v9_sweep.sh
#   ORIGINAL_WARMUP=100000 ORIGINAL_SIM=100000 bash projects/legacy_gru_prefetch/scripts/run_gru_v9_sweep.sh
...
if [ "$missing" -ne 0 ]; then
  ...
  echo "        PFETCH_DIR=/path/to/prefetch_lists bash projects/legacy_gru_prefetch/scripts/run_gru_v9_sweep.sh"
  exit 1
fi
...
for TRACE_NAME in $TRACES; do
  ...
  TRACE="$TRACE_NAME" \
  PFETCH="$PFETCH" \
  bash projects/legacy_gru_prefetch/scripts/run_gru_v9.sh
