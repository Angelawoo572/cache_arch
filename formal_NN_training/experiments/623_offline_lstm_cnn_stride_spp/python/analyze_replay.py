#!/usr/bin/env python3
"""Fail-closed analysis for matched 623 stride/SPP LSTM-versus-CNN tracks."""
import argparse
import csv
import gzip
import hashlib
...
def policy_for_method(method):
    if method in {"offline_stride", "offline_spp"}:
        return method.split("_", 1)[1]
    if method.startswith("offline_stride_"):
        return "stride"
    if method.startswith("offline_spp_"):
        return "spp"
    return ""


def model_tag_for_method(method):
    if method.startswith("offline_stride_") or method.startswith("offline_spp_"):
...
def add_comparison_metrics(rows, failures):
    ...
    for policy in POLICIES:
        ...
        for row in rows:
            ...
            bpi = 100.0 * (
                cache_block * q_accuracy * q_timeliness
            ) ** (1.0 / 3.0)
...
def main():
    ...
    for tag in model_tags:
        if not (
            tag.startswith("stride_lstm_")
            or tag.startswith("stride_cnn_")
            or tag.startswith("spp_lstm_")
            or tag.startswith("spp_cnn_")
        ):
    ...
    for method in methods:
        ...
        if policy and (row["matched"] <= 0 or row["emitted"] <= 0):
    ...
    for tag in model_tags:
        ...
        if list_path.is_file():
            normal_hashes[policy][tag] = sha256(list_path)
    ...
    for (policy, pair_id), members in pairs.items():
        ...
        if len(members) != 2 or families != {"lstm", "cnn"}:
            failures.append(
                "{} pair {} must contain one LSTM and one CNN".format(
                    policy, pair_id
                )
            )
            continue
        ...
        if max(counts) / float(min(counts)) > 1.15:
            failures.append(
                "{} pair {} parameters differ by >15%".format(
                    policy, pair_id
                )
            )
    ...
    fields = [
        ...
        "prefetch_duplicate_events", "event_selected_accuracy_proxy",
    ...
    payload = {
        ...
        "trace_selection": {
            "reason": (
                ...
                "versus no-prefetch while SPP is modestly positive; AMPM is "
                "excluded because its IPC is below no-prefetch."
            ),
            "historical_ipc_context": {
                "no_pref": 0.35321,
                "stride": 0.35340,
                "spp": 0.35391,
                "excluded_ampm": 0.34924,
            },
        },
        "primary_comparisons": {
            "stride_track": [
        ...
        "candidate_contract": (
            ...
            "outside its matched normal prefetcher bank."
        ),
        "cnn_contract": {
            ...
            "interpretation": (
                "one short filter slides one demand event at a time; output t "
                "uses only t-2, t-1, and t"
            ),
        },
        "metric_definitions": {
            "l2_load_miss_rate": (
            ...
            "balanced_parity_index": (
                "100 * (sqrt(q_miss_rate*q_coverage) * "
                "q_selected_accuracy * q_timeliness)^(1/3); each ratio is "
    ...
    if not failures:
        ...
        payload["presentation_decision_rule"] = {
            "cnn_wins": (
                ...
                "local address correlation is sufficient."
            ),
            "lstm_wins": (
