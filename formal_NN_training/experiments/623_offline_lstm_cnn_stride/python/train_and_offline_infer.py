#!/usr/bin/env python3
"""Train a matched LSTM or shallow sliding CNN gate for stride on 623.

The normal replay and its neural gate use the exact same live-stride candidate
stream and PC-line-occ replay
...
def load_candidate_bank(path, policy, rows):
    ...
    with gzip.open(path, "rt", newline="") as handle:
        ...
        for row in reader:
            ...
            if fill_level != 2:
                raise RuntimeError("stride candidate is not FILL_L2 at {}".format(index))
...
def main():
    ...
    metadata = {
        ...
        "forbidden_inputs": [
            ...
            "duplicate", "fill_level", "future_eval_rows",
        ],
        "normal_candidate_bank_is_fixed": True,
        "normal_candidate_bank_source": "live_stride_PF_requests",
        "normal_policy_private_state": [
            "PC-indexed_stride_tracker_table", "confidence", "last_stride",
        ],
