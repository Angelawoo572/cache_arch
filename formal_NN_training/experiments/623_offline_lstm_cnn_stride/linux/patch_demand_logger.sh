...
restore_clean_cache_cc() {
  ...
  # Do not use `git checkout -- src/cache.cc` here.  Some experimental server
  # clones have an older logger committed in HEAD, so checkout merely restores
  # another contaminated file.  Walk cache.cc history and restore the newest
  # blob that predates every known experimental logger.
