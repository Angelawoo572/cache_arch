...
def simulator_source():
    explicit = os.environ.get("ONLINE602_SIMULATOR")
    if explicit:
        candidate = Path(explicit).resolve()
        if not (candidate / "inc/instruction.h").is_file():
            raise RuntimeError("ONLINE602_SIMULATOR is not a simulator source: " + str(candidate))
        return candidate
    ...
    for candidate in candidates:
        if (candidate / "inc/instruction.h").is_file():
            return candidate
    raise RuntimeError("No actual ChampSim source found; set ONLINE602_SIMULATOR")
...
class MeasurementTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.temporary = tempfile.TemporaryDirectory(prefix="online602_measurement_")
        ...
        if "online602_tracker_count" not in text:
            anchor = "   ~StridePrefetcher();"
            if text.count(anchor) != 1:
                raise RuntimeError("actual Stride header anchor changed")
            text = text.replace(anchor, anchor + "\n   size_t online602_tracker_count() const { return trackers.size(); }", 1)
