#include "online602_hooks.h"
#include "online_observer_core.h"
#include "cache.h"
#include "ooo_cpu.h"
#include <cerrno>
#include <cstdlib>
...
namespace {
using online602_measurement::Observer;
...
uint64_t skip_count() { static uint64_t n=env_number("ONLINE602_SKIP_RECORDS"); return n; }
uint64_t max_count() { static uint64_t n=env_number("ONLINE602_MAX_RECORDS"); return n; }
...
void online602_begin(unsigned cpu) {
    State& s=states[cpu];
    if(s.began) throw std::runtime_error("online602 measurement boundary called twice");
    ...
    const char* path=std::getenv("ONLINE602_STATS");
    if(path && *path) {
        if(NUM_CPUS!=1) throw std::runtime_error("online602 observer requires a single core");
        s.out.open(path);
        if(!s.out) throw std::runtime_error("cannot open ONLINE602_STATS");
    }
    s.began=true;
    thresholds(cpu);
    online602_observe_cycle(cpu);
}

void online602_observe_cycle(unsigned cpu,bool final) {
    State& s=states[cpu];
    if(!s.began) return;
    ...
    if(valid!=o.occupancy) throw std::runtime_error("online602 valid-line occupancy mismatch");
    if(s.out) {
        online602_measurement::Cohort outcomes;
        ...
        if(outcomes.enqueued != outcomes.timely+outcomes.late+outcomes.unused+
            ...
            throw std::runtime_error("online602 prefetch lifecycle conservation mismatch");
        ...
        const std::string engine=online602_engine_json(cpu);
...
void online602_fill(CACHE* c,unsigned set,unsigned way,const PACKET* packet) {
    if(!active(c)) return;
    states[c->cpu].obs->fill(uint64_t(set)*c->NUM_WAY+way,c->block[set][way].valid,
                            packet->type,packet->online602_measurement_id,cycles(c->cpu));
    thresholds(c->cpu);
}
void online602_invalidate(CACHE* c,unsigned set,unsigned way) {
    if(active(c)) states[c->cpu].obs->invalidate(uint64_t(set)*c->NUM_WAY+way,cycles(c->cpu));
}
void online602_demand_hit(CACHE* c,unsigned set,unsigned way) {
    if(active(c)) states[c->cpu].obs->demand(uint64_t(set)*c->NUM_WAY+way,cycles(c->cpu));
}
void online602_late(CACHE* c,const PACKET* packet) {
    if(active(c)) states[c->cpu].obs->resolve(packet->online602_measurement_id,1,cycles(c->cpu));
}
void online602_enqueue(CACHE* c,PACKET* packet) {
    if(active(c)) packet->online602_measurement_id=states[c->cpu].obs->enqueue(instructions(c->cpu));
}
void online602_redundant(CACHE* c,const PACKET* packet,bool cache_hit) {
    if(active(c)) states[c->cpu].obs->resolve(packet->online602_measurement_id,cache_hit?3:4,cycles(c->cpu));
}
...
void online602_skip(FILE* file,bool cloudsuite,unsigned cpu) {
    ...
    if(cloudsuite) online602_measurement::skip_exact_records<cloudsuite_instr>(file,skip_count());
    else online602_measurement::skip_exact_records<input_instr>(file,skip_count());
    std::cout << "online602_trace_skip_records " << skip_count()
              << " reader_record_bytes " << record_bytes << " cpu " << cpu << std::endl;
}
bool online602_read_allowed(unsigned cpu) { return !max_count() || records[cpu]<max_count(); }
void online602_record_read(unsigned cpu) { ++records[cpu]; }
bool online602_bounded_trace() { return max_count()!=0; }
