#!/usr/bin/env bash
# Linux stages for the isolated 602 address-only LSTM-versus-streamer experiment.
...
assert_live_streamer() {
  ...
  grep -Fqi "adding L2C_PREFETCHER: streamer" "$log" || {
    echo "[error] live streamer was not registered; refusing an inactive reference" >&2
    exit 3
  }
  grep -Fq "streamer_num_trackers 64" "$log" || {
    echo "[error] live streamer did not use 64 trackers" >&2
    exit 3
  }
  grep -Fq "streamer_pref_degree 5" "$log" || {
    echo "[error] live streamer did not use degree 5" >&2
    exit 3
  }
  grep -Eq '^streamer\.pred\.total [1-9][0-9]*$' "$log" || {
    echo "[error] live streamer generated zero prefetches" >&2
    exit 3
  }
}
  ...
  case "$method" in
    no_pref)
      DEMAND_EVENT_LOG="$raw" "$BIN" --l2c_prefetcher_types=none \
        ...
        -traces "$TRACE_FILE" > "$log" 2>&1
      ;;
    live_streamer_reference)
      # Explicit top-level flags are required by this ChampSim parser.
      DEMAND_EVENT_LOG="$raw" "$BIN" --l2c_prefetcher_types=streamer \
        --streamer_num_trackers=64 --streamer_pref_degree=5 \
        --warmup_instructions=25000000 --simulation_instructions=25000000 \
        -traces "$TRACE_FILE" > "$log" 2>&1
      assert_live_streamer "$log"
      ;;
    offline_streamer)
      local list
      list="$(colab_dir "$BASE_MODEL_TAG")/offline_streamer.replay.csv"
...
replay() {
  ...
  require_colab_outputs
  run_method no_pref
  run_method live_streamer_reference
  run_method offline_streamer
  for tag in "${MODEL_TAGS[@]}"; do
