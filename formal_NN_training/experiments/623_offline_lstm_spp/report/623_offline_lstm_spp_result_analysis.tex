...
are from the completed historical artifact shown below.  The current implementation is
\texttt{global\_chronological\_lstm\_finite\_joint\_rank\_v23} with default run
\texttt{623\_offline\_lstm\_spp\_finite\_joint\_rank\_v23\_seed7}; its replay
results are pending and must not be inferred from these tables.  Because the
recorded fill-callback stream comes from source SPP, v23 supports a matched-input
...
Current output model & one finite joint softmax directly predicts STOP or EMIT(delta, fill).  The exact finite support is the maximum TRAIN teacher-action count; shorter sequences receive STOP on every available tail rank, while maximum-length sequences end at finite support.  There are no separate gate, count, delta, or fill argmaxes and no prior-action feedback.\\
...
Historical run artifact & prior completed runs provide diagnosis only.  ``PASS'' on those artifacts does not certify v23 or useful neural behavior.\\
\end{tabularx}
\end{infobox}

\begin{infobox}[colback=blue!5,colframe=blue]
\textbf{v23 protocol (results pending).}  Hidden sizes 8, 16, 32, 64, and 128
are trained because 602 and 623 showed non-monotone capacity behavior.  One
joint token cross-entropy is grouped by STOP, EMIT-L2, and EMIT-LLC using only
TRAIN rank-slot counts.  Decode subtracts the log TRAIN group weight and takes
one deterministic joint argmax.  Guard selection remains lexicographic; no
heterogeneous average, probability threshold, page rule, or normal request
budget is used.  A TRAIN-modal-delta/all-LLC policy is replayed separately as
an explicitly non-neural diagnostic control.
\end{infobox}
