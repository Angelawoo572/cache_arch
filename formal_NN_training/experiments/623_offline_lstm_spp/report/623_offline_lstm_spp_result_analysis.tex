...
\texttt{global\_chronological\_lstm\_hurdle\_log\_count\_v22} with default run
\texttt{623\_offline\_lstm\_spp\_hurdle\_log\_count\_vocab\_v22\_seed7}; its replay
results are pending and must not be inferred from these tables.  Because the
recorded fill-callback stream comes from source SPP, v22 supports a matched-input
...
Current output model & a natural-frequency ZERO/POSITIVE hurdle and conditional positive log-count choose cardinality.  Each scheduled rank independently chooses a dynamic TRAIN-only exact-delta class or OTHER coordinate; a target/rank-conditioned fill head uses deterministic MAP.  There is no prior-action feedback.\\
...
Historical run artifact & \texttt{623\_offline\_lstm\_spp\_variable\_delta\_free\_running\_v11\_seed7}.  ``PASS'' certifies that historical experiment contract; it does not certify v22 or useful neural behavior.\\
\end{tabularx}
\end{infobox}

\begin{infobox}[colback=blue!5,colframe=blue]
\textbf{v22 protocol (results pending).}  Hidden sizes 8, 16, 32, 64, and 128
are trained because 602 showed non-monotone capacity and fill-collapse effects.
The ZERO/POSITIVE hurdle uses natural-frequency unweighted categorical loss,
and positive callbacks use unweighted smooth-L1 on log action count;
the rare fill classes use only TRAIN-derived inverse-frequency weights.
Inference is deterministic argmax.  The guard key is lexicographic over joint
target/fill F1, target F1, L2-joint F1, trigger F1, exact count, fill accuracy,
train loss, and epoch; no heterogeneous average is formed.  Evaluation never
selects a checkpoint, but prior 623 iterations have already inspected this
benchmark region, so the replay is not a pristine once-only confirmatory test.
\end{infobox}
...
\item Because all three displayed historical LSTM capacities collapse identically and target F1 is zero, the immediate evidence points to action-count/label imbalance or decoder learning; the table does not establish a recurrent-versus-convolutional architecture result.
\end{itemize}
\end{infobox}
