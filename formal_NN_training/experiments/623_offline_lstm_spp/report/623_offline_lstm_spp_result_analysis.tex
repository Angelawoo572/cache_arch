...
\texttt{global\_chronological\_lstm\_independent\_actions\_v20} with default run
\texttt{623\_offline\_lstm\_spp\_independent\_vocab\_v20\_seed7}; its replay
results are pending and must not be inferred from these tables.  Because the
recorded fill-callback stream comes from source SPP, v20 supports a matched-input
...
Historical run artifact & \texttt{623\_offline\_lstm\_spp\_variable\_delta\_free\_running\_v11\_seed7}.  ``PASS'' certifies that historical experiment contract; it does not certify v20 or useful neural behavior.\\
\end{tabularx}
\end{infobox}
