...
\texttt{global\_chronological\_lstm\_rank\_stop\_emit\_v21} with default run
\texttt{623\_offline\_lstm\_spp\_stop\_emit\_vocab\_v21\_seed7}; its replay
results are pending and must not be inferred from these tables.  Because the
recorded fill-callback stream comes from source SPP, v21 supports a matched-input
...
Current output model & each rank independently predicts STOP or EMIT from the chronological callback state and rank code.  EMIT chooses a dynamic TRAIN-only exact-delta class or OTHER coordinate; a target/rank-conditioned fill head uses deterministic MAP.  Every teacher action and the terminal STOP are supervised, with no separate count head or prior-action feedback.\\
...
Historical run artifact & \texttt{623\_offline\_lstm\_spp\_variable\_delta\_free\_running\_v11\_seed7}.  ``PASS'' certifies that historical experiment contract; it does not certify v21 or useful neural behavior.\\
\end{tabularx}
\end{infobox}

\begin{infobox}[colback=blue!5,colframe=blue]
\textbf{v21 protocol (results pending).}  Hidden sizes 8, 16, 32, 64, and 128
are trained because 602 showed non-monotone capacity and fill-collapse effects.
The naturally denser STOP/EMIT token stream uses unweighted categorical loss;
