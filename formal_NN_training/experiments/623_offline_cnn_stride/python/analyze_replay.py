#!/usr/bin/env python3
"""Fail-closed analysis for the standalone 623 Stride/causal-CNN track."""
import argparse
import csv
import gzip
import hashlib
...
DEFAULT_MODEL_TAGS = (
    "independent_delta_stride_cnn_c7,independent_delta_stride_cnn_c15,"
    "independent_delta_stride_cnn_c24"
)
EXPECTED_POINTS = {
    ("cnn", 7): "p0",
    ("cnn", 15): "p1",
...
def validate_metadata(metadata, tag, inputs, failures):
    ...
    if point is None:
        failures.append("{} is not a pinned CNN point".format(tag))
    else:
...
def main():
    ...
    for tag in model_tags:
        if not tag.startswith("independent_delta_stride_cnn_"):
