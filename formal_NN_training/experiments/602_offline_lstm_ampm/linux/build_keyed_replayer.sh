#!/usr/bin/env bash
# Build one binary containing none, live AMPM, and keyed ListReplayer.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
CHAMP_DIR="${CHAMP_DIR:-$ROOT/external/ChampSim}"
GENERATED="$CHAMP_DIR/prefetcher/offline_602_ampm_replayer.l2c_pref"
TEMPLATE="$CHAMP_DIR/prefetcher/.offline_602_ampm_replayer.tmp"
HEADER="$CHAMP_DIR/inc/list_replayer.h"
SOURCE="$CHAMP_DIR/prefetcher/list_replayer.cc"
BUILT="$CHAMP_DIR/bin/perceptron-no-offline_602_ampm_replayer-no-ship-1core"
OUT="${OUT:-$CHAMP_DIR/bin/champsim.602_offline_ampm_replay}"
...
( cd "$CHAMP_DIR" && ./build_champsim.sh no offline_602_ampm_replayer no 1 )
