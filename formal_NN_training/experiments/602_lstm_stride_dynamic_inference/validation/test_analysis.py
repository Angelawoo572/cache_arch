...
class MetricFixtures(unittest.TestCase):
    ...
    def test_real_parser_and_end_to_end_fixture(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            for name, cycles_per_window in (("none", 4_000_000), ("stride", 3_000_000), ("frozen_i1m", 2_000_000)):
                run = root / name
                run.mkdir()
            ...
            self.assertTrue(all(row["arm"] in ("no_pref", "stride", "frozen") for row in payload["summary"]))
            frozen = next(r for r in payload["summary"] if r["arm"] == "frozen")
            self.assertEqual(frozen["cycles_saved_vs_stride"], 3_000_000)
            self.assertNotIn("learning_milestones", payload)

    def test_non_frozen_run_cannot_enter_report(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            wrong = root / "misleading_frozen_name"
            wrong.mkdir()
            (wrong / "run.json").write_text(json.dumps(dict(arm="online_random")))
            payload = analysis.analyze(root, root / "tables")
            self.assertEqual(payload["total_discovered_runs"], 0)
            self.assertEqual(payload["excluded_non_frozen_directories"], 1)

    def test_frozen_deployment_excludes_legacy_unused_fields(self):
        cleaned = analysis.deployment_fields(dict(weight_bytes=7632, training_examples_bytes_configured=0, completed_updates=0, teacher_calls=0, published_versions=0))
        self.assertEqual(cleaned, dict(weight_bytes=7632))
