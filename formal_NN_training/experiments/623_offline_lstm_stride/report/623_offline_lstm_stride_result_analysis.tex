...
\texttt{pc\_keyed\_raw\_rank\_stop\_emit\_v21} with default run
\texttt{623\_offline\_lstm\_stride\_rank\_stop\_emit\_v21\_seed7}; its replay results
...
Current output model & each rank independently predicts STOP or EMIT from the callback state and rank code.  EMIT chooses a dynamic TRAIN-only exact-delta class or OTHER signed-log coordinate, all relative to the current demand.  Every teacher action and the terminal STOP are supervised; there is no separate count head or prior-action feedback.\\
...
Historical run artifact & \texttt{623\_offline\_lstm\_stride\_variable\_delta\_free\_running\_v9\_seed7}.  ``PASS'' certifies that historical experiment contract; it does not certify v21 or useful neural behavior.\\
\end{tabularx}
\end{infobox}
...
STOP/EMIT labels receive the unique TRAIN-derived inverse-frequency class
weight that gives the two token classes equal aggregate loss mass; inference
still uses raw argmax and no threshold.  The guard key is lexicographic
