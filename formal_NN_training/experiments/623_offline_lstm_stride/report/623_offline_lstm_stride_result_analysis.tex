...
\texttt{pc\_keyed\_raw\_hurdle\_count\_rank\_delta\_v22\_reused\_v23} with default run
\texttt{623\_offline\_lstm\_stride\_prior\_corrected\_hurdle\_v23\_seed7}; its replay results
...
Current output model & the v22 checkpoint, count head, and rank-delta heads are reused byte-for-byte.  Only hurdle decode changes from weighted-logit argmax to \(z_c-\log w_c\), exactly undoing TRAIN inverse-frequency weighting before deterministic argmax.\\
...
reuse the corresponding v22 checkpoint and training history.  For every point,
the notebook first reproduces the v22 raw-decode replay list byte-for-byte and
then performs one prior-corrected decode.  No model is retrained, no checkpoint
is reselected, and no threshold or request budget is introduced.  Parent and
reproduced artifact SHA-256 values are fail-closed metadata requirements.
\end{infobox}
