#!/usr/bin/env python3
"""Train a matched LSTM or shallow sliding CNN gate over SPP candidates on 623.

The normal replay and its neural gate use the exact same causal live-SPP
candidate actions and PC-line-occ replay
    ...
    with gzip.open(path, "rt", newline="") as handle:
        ...
        for row in reader:
            ...
            if fill_level not in (2, 4):
                raise RuntimeError("invalid SPP fill level at {}".format(index))
            bank[index].append((as_int(row["pf_line"]), fill_level))
...
def candidate_arrays(rows, bank):
    ...
    fill_level = np.zeros((len(rows), max_candidates), dtype=np.int8)
    for index, (_, line, _) in enumerate(rows):
        for slot, (pf_line, pf_fill_level) in enumerate(bank[index]):
            ...
            fill_level[index, slot] = pf_fill_level
    ...
    return candidate, valid, target, fill_level
...
def write_replay(path, rows, selected, valid, target, fill_level):
    entries = 0
    triggers = 0
    with Path(path).open("w", newline="") as handle:
        ...
        for index, (pc, line, occurrence) in enumerate(rows):
            ...
            for slot in slots:
                writer.writerow([
                    ...
                    int(fill_level[index, slot]),
                ])
                entries += 1
    return entries, triggers


def selected_fill_counts(selected, valid, fill_level):
    chosen = fill_level[selected & valid]
    return {
        "FILL_L2": int((chosen == 2).sum()),
        "FILL_LLC": int((chosen == 4).sum()),
    }
...
def main():
    ...
    for role in ("train", "guard", "eval"):
        candidate[role], valid[role], target[role], fill_level[role] = candidate_arrays(
    ...
    metadata = {
        ...
        "forbidden_inputs": [
            ...
            "duplicate", "fill_level", "spp_confidence",
            "signature_table_state", "pattern_table_state", "GHR_state",
            "prefetch_filter_feedback", "future_eval_rows",
        ],
        "normal_candidate_bank_is_fixed": True,
        "normal_candidate_bank_source": "causal_live_SPP_dev2_PF_requests",
        ...
        "offline_normal_fill_level_counts": selected_fill_counts(
            normal_selected, valid["eval"], fill_level["eval"]
        ),
        "offline_nn_fill_level_counts": selected_fill_counts(
            selected_eval, valid["eval"], fill_level["eval"]
        ),
