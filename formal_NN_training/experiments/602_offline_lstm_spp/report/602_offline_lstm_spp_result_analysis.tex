...
\item h8 is not an input, gate, count, target, or replay collapse. It predicts actions on every callback, has 100\% teacher-trigger recall, 74.86\% target F1, and emits 1.897M accepted actions. Its distinctive failure is fill selection: every emitted action is \texttt{FILL\_LLC}, leaving zero \texttt{FILL\_L2}, zero L2 prefetch fills, and zero L2 useful hits.
...
capacity predicts the positive categorical mode on 100\%. This is a learned
argmax operating point, not a probability threshold or hard-coded degree.
