...
GENERATED="$CHAMP_DIR/prefetcher/offline_602_direct_spp_replayer.l2c_pref"
TEMPLATE="$CHAMP_DIR/prefetcher/.offline_602_direct_spp_replayer.tmp"
HEADER="$CHAMP_DIR/inc/list_replayer_fill.h"
SOURCE="$CHAMP_DIR/prefetcher/list_replayer_fill.cc"
BUILT="$CHAMP_DIR/bin/perceptron-no-offline_602_direct_spp_replayer-no-ship-1core"
OUT="${OUT:-$CHAMP_DIR/bin/champsim.602_direct_spp_replay}"
...
if [[ -e "$HEADER" || -e "$SOURCE" ]]; then
  if [[ -f "$HEADER" && -f "$SOURCE" ]] \
    ...
    echo "[cleanup] removing identical files left by an interrupted 602 SPP build"
    rm -f "$HEADER" "$SOURCE"
  else
...
( cd "$CHAMP_DIR" && ./build_champsim.sh no offline_602_direct_spp_replayer no 1 )
