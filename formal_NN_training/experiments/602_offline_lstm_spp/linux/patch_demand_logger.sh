...
# the matched 602 SPP experiment.  Instrumentation only: it does not alter the
...
restore_clean_cache_cc() {
  ...
  mkdir -p "$save_dir"
  cp -f "$CACHE_CC" "$save_dir/cache.cc.before_602_spp_reset.cc"
  git -C "$CHAMP_DIR" diff -- src/cache.cc \
    > "$save_dir/cache.cc.before_602_spp_reset.patch" || true
  ...
  while IFS= read -r commit; do
    ...
    if ! grep -Eq 'DEMAND_EVENT_LOG_SCHEMA_(623_|602_SPP_)|demand_event_log_demand|RESIDUAL_AUDIT_LOG' "$tmp"; then
      mv -f "$tmp" "$CACHE_CC"
      ...
      echo "[clean-restore] original file: $save_dir/cache.cc.before_602_spp_reset.cc"
      return 0
    fi
...
elif ! grep -Fq "DEMAND_EVENT_LOG_SCHEMA_602_SPP_V1" "$CACHE_CC" \
    && grep -Eq 'DEMAND_EVENT_LOG_SCHEMA_(623_|602_SPP_)|demand_event_log_demand|RESIDUAL_AUDIT_LOG' "$CACHE_CC"; then
...
if [[ "$CACHE_CC_CLEAN_RESTORED" != "1" ]] \
    && ! grep -Fq "DEMAND_EVENT_LOG_SCHEMA_602_SPP_V1" "$CACHE_CC" \
...
revision_marker = "DEMAND_EVENT_LOG_SCHEMA_602_SPP_V1"
if revision_marker in s:
    print("[skip] 602 SPP v1 demand/fill causal logger already present", p)
    raise SystemExit(0)
...
helper = r'''
// DEMAND_EVENT_LOG_SCHEMA_602_SPP_V1
// -----------------------------------------------------------------------------
// Causally keyed raw L2 demand/PF logger for the 602 SPP experiment.
...
static const char demand_event_logger_schema[] = "602_spp_causal_trigger_fill_v1";
...
expected = {
    "DEMAND_EVENT_LOG_SCHEMA_602_SPP_V1": 1,
    '"602_spp_causal_trigger_fill_v1"': 1,
...
if bad:
    raise SystemExit("[error] installed 602 SPP v1 logger structural audit failed: {}".format(bad))
print("[PASS] installed 602 SPP v1 demand/fill logger structural audit")
PY
