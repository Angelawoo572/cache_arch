#!/usr/bin/env bash
# Independent 602 track: normal SPP versus direct SPP-interface LSTM only.
set -euo pipefail
...
BIN="${BIN:-$CHAMP_DIR/bin/champsim.602_direct_spp_lstm_replay}"
...
build() {
  ...
  (
    ...
    flock -x 9
    echo "[build-lock] acquired by 602 SPP run $RUN_ID"
    audit_spp_source
    ...
    echo "[build-lock] 602 direct-SPP build complete"
  ) 9>"$BUILD_LOCK"
}
...
expected_parameters = expected_parameter_counts.get(metadata.get("model_size"))
...
if bad:
    raise SystemExit("invalid 602 SPP metadata: {}".format(bad))
PY
}
