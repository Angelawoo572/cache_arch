#!/usr/bin/env bash
# Isolated nohup launcher; creates RUN_DIR before opening the log.
set -euo pipefail
...
nohup env -i   HOME="$HOME"   PATH="$PATH"   RUN_ID="$RUN_ID"   RUN_DIR="$RUN_DIR"   STAGE="$STAGE"   FORCE="${FORCE:-1}"   BUILD="${BUILD:-1}"   JOBS="${JOBS:-8}"   RESET_PATCH="${RESET_PATCH:-1}"   bash "$EXP/linux/run_server.sh"   > "$LOG" 2>&1 < /dev/null &
pid=$!
printf '%s\n' "$pid" > "$PID_FILE"
