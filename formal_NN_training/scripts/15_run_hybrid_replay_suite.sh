...
prepare_trace () {
  ...
  local packed_dir="$ROOT/formal_NN_training/artifacts/packed/$tag"
  ...
  if [ "$FORCE_PREPARE" = "1" ]; then
    python3 formal_NN_training/scripts/07_prepare_actions_for_replay.py --trace "$trace" --restore-packed
  elif [ -s "$actions" ]; then
    python3 formal_NN_training/scripts/07_prepare_actions_for_replay.py --trace "$trace"
...
if [ "$RUN_FIGURES" = "1" ]; then
  python3 formal_NN_training/scripts/16_make_hybrid_replay_figures.py \
