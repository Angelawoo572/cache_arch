...
LOG_DIR = Path("formal_NN_training/results/capacity_sweep/logs")
OUT = Path("formal_NN_training/results/capacity_sweep/capacity_sweep_602_619.csv")
...
def key(r):
    ...
    return (r["trace"], cap_order.get(r["l2_capacity"], 99), method_order.get(r["method"], 99))
...
with OUT.open("w", newline="") as f:
    ...
    for r in sorted(rows, key=key):
        rr = dict(r)
        for k in ["ipc", "speedup_vs_no_prefetch", "useful_per_issued", "useful_over_useful_plus_useless"]:
            if rr.get(k) is None:
                rr[k] = "NA"
            else:
                rr[k] = "{:.8f}".format(rr[k])
