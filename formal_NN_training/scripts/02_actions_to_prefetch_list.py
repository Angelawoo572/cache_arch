...
The outcome-aware trainer exports pred_good_prefetch_prob and candidate_addr.
...
Optional timing filters can further restrict emitted candidates using columns such
as pred_timing_bin or label_timing. This is useful for replaying timing-aware
keep/drop policies without retraining the notebook model.
"""
import argparse
import csv
...
def main():
    ...
    ap.add_argument(
        "--timing-column",
        default="",
        help="Timing column to filter on. Default with timing bounds: pred_timing_bin, then pred_timing, then label_timing.",
    )
    ap.add_argument("--timing-min-bin", type=int, default=None, help="Keep only rows with timing bin >= this value.")
    ap.add_argument("--timing-max-bin", type=int, default=None, help="Keep only rows with timing bin <= this value.")
    args = ap.parse_args()
    ...
    use_timing_filter = args.timing_min_bin is not None or args.timing_max_bin is not None
    timing_col = ""
    if use_timing_filter:
        timing_col = args.timing_column or first(fields, ["pred_timing_bin", "pred_timing", "label_timing"])
        if not timing_col or timing_col not in set(fields):
            raise SystemExit("[error] timing filter requested, but no timing column found. Expected pred_timing_bin or label_timing.")

    emitted = set()
    timing_seen = Counter()
    timing_kept = Counter()
    n = skip_policy = skip_conf = skip_bypass = skip_timing = skip_self = skip_addr = 0
    for row in reader:
        n += 1
        ...
        if use_timing_filter:
            tbin = to_int(row.get(timing_col), -1)
            timing_seen[tbin] += 1
            if args.timing_min_bin is not None and tbin < args.timing_min_bin:
                skip_timing += 1
                continue
            if args.timing_max_bin is not None and tbin > args.timing_max_bin:
                skip_timing += 1
                continue
            timing_kept[tbin] += 1
    ...
    if use_timing_filter:
        print(f"[timing] column={timing_col} min={args.timing_min_bin} max={args.timing_max_bin}")
        print(f"[timing_seen_after_policy] {dict(sorted(timing_seen.items()))}")
        print(f"[timing_kept_after_filter] {dict(sorted(timing_kept.items()))}")
    print(f"[rows]   input={n} emitted={len(pairs)} unique_pairs={len(emitted)}")
    print(f"[skip]   policy={skip_policy} low_conf={skip_conf} bypass={skip_bypass} timing={skip_timing} self={skip_self} bad_addr={skip_addr}")
    if pairs:
