...
def resolve_oracle(path: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if pc_col is None or line_col is None:
        raise ValueError(f"{path}: need PC and line columns; found {list(frame.columns)}")
    ...
    if not np.array_equal(demand_idx, expected):
        raise ValueError(f"{path}: demand_idx must be contiguous 0..N-1 to make a safe sidecar")
...
def skip_records(handle, n_records: int) -> None:
    ...
    while remaining:
        ...
        if got == 0:
            raise RuntimeError(f"trace ended while skipping warmup: skipped {n_records - remaining}/{n_records}")
    ...
    with opener(trace_path, "rb") as handle:
        ...
        while event < n:
            ...
            if len(record) != INPUT_INSTR_RECORD.size:
                raise RuntimeError(f"partial input_instr record at dynamic instruction {dynamic_seq}")
            ...
            if pc == int(oracle_pc[event]) and source_lines:
                ...
                if source_slot >= 0:
                    ...
                    if parent is not None:
                        ...
                        dep_parent_dynamic_gap[event] = max(0, dynamic_seq - parent_seq)
...
def choose_oracle(trace_stem: str, oracle: Optional[str], oracle_dir: Optional[str]) -> Path:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"expected exactly one oracle under {root} containing {trace_stem!r}; found {matches}")
    return matches[0]
